...
namespace Parlex
{
    public static class NfaExtensions
    {
        ...
        {
            if (!nfa.States.Contains(state)) throw new InvalidNfaOperationException("The specified State is not in the Nfa");

            var newState = new Nfa<TAlphabet>.State();
            if (nfa.StartStates.Contains(state)) nfa.StartStates.Add(newState);
            if (nfa.AcceptStates.Contains(state)) nfa.AcceptStates.Add(newState);
            ...
            foreach (var transition in nfa.GetTransitions())
            {
        ...
        /// <summary>
        /// The specified state is replaced by new states which each have transitions going to only the states as grouped by toGroups.
        /// Any state, for which a transition exists to that state from the specified state, that is not specified in toGroups will be placed in its own group.
            ...
            IEnumerable<IEnumerable<Nfa<TAlphabet>.State>> toGroups = null)
        {
            ...
            if (toGroups == null) toGroups = new List<List<Nfa<TAlphabet>.State>>();
            if (!nfa.States.Contains(state)) throw new InvalidNfaOperationException("The specified State is not in the Nfa");

            //cache some lists
            var cachedTransitionGroups = toGroups.Select(x => x.Select(y => y).ToList()).ToList();
            var allNonSelfToStates = nfa.GetTransitions().Where(x => x.FromState == state).Select(x => x.ToState).Where(x => x != state).ToList();
            ...
            if (cachedTransitionGroups.SelectMany(x => x).Contains(state)) throw new InvalidNfaOperationException("The specified state may not be included in the specified toGroups");

            //Put each unspecified to-state in its own group
            foreach (var ungrouped in allNonSelfToStates.Except(cachedTransitionGroups.SelectMany(x => x)))
            {
                cachedTransitionGroups.Add(new List<Nfa<TAlphabet>.State>(new[] { ungrouped }));
            }

            var isStart = nfa.StartStates.Contains(state);
            var isAccept = nfa.AcceptStates.Contains(state);
            ...
            foreach (var cachedTransitionGroup in cachedTransitionGroups)
            {
                ...
                nfa.States.Add(newState);
                if (isStart) nfa.StartStates.Add(newState);
                if (isAccept) nfa.AcceptStates.Add(newState);
                //hook the new state up to its toGroup by adding the appropriate transitions

                foreach (var toState in cachedTransitionGroup)
                {
                    Nfa<TAlphabet>.State state1 = toState;
                    foreach (var transition in nfa.GetTransitions().Where(x => x.ToState == state1))
                    {
            ...
            foreach (var incomingTransition in nfa.GetTransitions().Where(x => x.ToState == state && x.FromState != state))
            {
            ...
            foreach (var selfTransition in nfa.GetTransitions().Where(x => x.ToState == state && x.FromState == state))
            {
                foreach (var result in results)
                {
            ...
            if (isStart) nfa.StartStates.Remove(state);
            if (isAccept) nfa.AcceptStates.Remove(state);
            return results;
        }

        /// <summary>
        /// The specified state is replaced by new states which each have transitions coming from only the states as grouped by fromGroups.
        /// Any state, for which a transition exists from that state to the specified state, that is not specified in fromGroups will be placed in its own group.
        /// This is a behavior preserving transformation.
        /// This function is robust and will throw an InvalidNfaOperationException if the inputs are not sane.
        /// </summary>
        /// <typeparam name="TAlphabet"></typeparam>
            ...
            IEnumerable<IEnumerable<Nfa<TAlphabet>.State>> fromGroups = null)
        {
            //do some input sanity checks
            if (state == null) throw new ArgumentNullException("state");
            if (fromGroups == null) fromGroups = new List<List<Nfa<TAlphabet>.State>>();
            if (!nfa.States.Contains(state)) throw new InvalidNfaOperationException("The specified State is not in the Nfa");

            //cache some lists
            var cachedTransitionGroups = fromGroups.Select(x => x.Select(y => y).ToList()).ToList();
            var allNonSelfFromStates = nfa.GetTransitions().Where(x => x.ToState == state).Select(x => x.FromState).Where(x => x != state).ToList();
            ...
            if (cachedTransitionGroups.SelectMany(x => x).Contains(state)) throw new InvalidNfaOperationException("The specified state may not be included in the specified fromGroups");

            //Put each unspecified from-state in its own group
            foreach (var ungrouped in allNonSelfFromStates.Except(cachedTransitionGroups.SelectMany(x => x)))
            {
                cachedTransitionGroups.Add(new List<Nfa<TAlphabet>.State>(new[] { ungrouped }));
            }
            ...
            foreach (var cachedTransitionGroup in cachedTransitionGroups)
            {
                ...
                foreach (var fromState in cachedTransitionGroup)
                {
                    Nfa<TAlphabet>.State state1 = fromState;
                    foreach (var transition in nfa.GetTransitions().Where(x => x.FromState == state1))
                    {
            ...
            foreach (var outgoingTransition in nfa.GetTransitions().Where(x => x.FromState == state && x.ToState != state))
            {
                foreach (var result in results)
                {
        ...
        public static void PeelBack<TAlphabet>(this Nfa<TAlphabet> nfa, Nfa<TAlphabet>.State state)
        {
            ...
            var splits = SplitFroms(nfa, state);
            foreach (var state1 in splits)
            {
            Restart:
        ...
        public static IEnumerable<IEnumerable<Nfa<TAlphabet>.State>> GetMacroCycles<TAlphabet>(this Nfa<TAlphabet> nfa)
        {
            ...
            var results = nfa.GetCycles().Select(x => new HashSet<Nfa<TAlphabet>.State>(x)).ToList();
        ...
        /// <returns></returns>
        static bool HasUnaryRoute<TAlphabet>(this Nfa<TAlphabet> nfa, Nfa<TAlphabet>.State from, Nfa<TAlphabet>.State to, IEnumerable<Nfa<TAlphabet>.State> subgraph = null) {
            if (subgraph == null) subgraph = new List<Nfa<TAlphabet>.State>();
            var routes = nfa.GetRoutes(from, to, new HashSet<Nfa<TAlphabet>.State>(nfa.States.Except(subgraph)));
            if (routes.Count() != 1) return false;
            foreach (var state in routes.First()) {
                if (state == to) continue;
                if (nfa.TransitionFunction[state].SelectMany(x => x.Value).Count() != 1) return false;
            }
            return true;
        }
    }
