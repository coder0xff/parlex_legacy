    ...
    public class OldParser {
        private class SubMatchChain : IEquatable<SubMatchChain> {
            ...
            internal class Entry : IEquatable<Entry> {
                ...
                internal Entry(OldProduction product, int lengthInParsedText) {
                    Product = product;
        ...
        private readonly Dictionary<OldProduction, Dictionary<int /*length*/, SubMatchChain>>[] _completedProductMatches;
        private readonly Int32[] _textCodePoints;
        private readonly StrictPartialOrder<OldProduction> _precedence;
        private readonly ParseResult _result;

        private OldParser(String text, OldProduction toMatch, StrictPartialOrder<OldProduction> precedence) {
            _textCodePoints = text.GetUtf32CodePoints();
            _completedProductMatches = new Dictionary<OldProduction, Dictionary<int, SubMatchChain>>[_textCodePoints.Length];
            for (int initCollections = 0; initCollections < _textCodePoints.Length; initCollections++) {
                _completedProductMatches[initCollections] = new Dictionary<OldProduction, Dictionary<int, SubMatchChain>>();
            }
            _precedence = precedence;
            MatchProduct(toMatch, 0, new Dictionary<OldProduction, DependencyMediator>());
        ...
        static public ParseResult Parse(String text, String productName, CompiledGrammar grammar) {
            ...
            return new OldParser(text, toMatch, grammar._precedences)._result;
        }
        ...
        private class SequenceMatchState {
            private readonly OldParser _oldParser;
            ...
            internal SequenceMatchState(OldParser oldParser, CompiledGrammar.NfaSequence sequence, int counter, int textToParseIndex, int sequenceStartingTextToParseIndex, CompiledGrammar.NfaSequence.ProductReference neededProduct, List<SubMatchChain.Entry> matchesThusFar, DependencyMediator dependencyMediator) {
            ...
            internal void DependencyFulfilled(int length) {
                ...
                if (_neededProduct.IsRepetitious) {
                    CreateNextStates(_counter, nextTextToParseIndex, nextMatchesThusFar, new Dictionary<OldProduction, DependencyMediator>());
                } else {
                    if (_neededProduct.ExitSequenceCounter == _sequence.SpanStart + _sequence.SpanLength) {
                        ...
                        CreateNextStates(_neededProduct.ExitSequenceCounter, nextTextToParseIndex, nextMatchesThusFar, new Dictionary<OldProduction, DependencyMediator>());
                    }
                }
            }
            ...
            internal void DependencyUnfulfilled() {
                if (_neededProduct.IsRepetitious) {
                    if (_neededProduct.ExitSequenceCounter == _sequence.SpanStart + _sequence.SpanLength) {
                        ...
                        CreateNextStates(_neededProduct.ExitSequenceCounter, _textToParseIndex, _matchesThusFar, new Dictionary<OldProduction, DependencyMediator>());
                    }
                } else {
            ...
            internal void CreateNextStates(int nextCounter, int nextTextToParseIndex, List<SubMatchChain.Entry> nextMatchesThusFar, Dictionary<OldProduction, DependencyMediator> dependencyMediators) {
                ...
                foreach (var nextProductReference in nextProducts) {
                    ...
                    var nextSequenceMatchState = new SequenceMatchState(_oldParser, _sequence, nextCounter, nextTextToParseIndex, _sequenceStartingTextToParseIndex, nextProductReference, nextMatchesThusFar, _dependencyMediator);
                    HashSet<int> matchResults = null;
                    if (!dependencyMediators.ContainsKey(nextProduct)) {
                        matchResults = _oldParser.MatchProduct(nextProduct, nextTextToParseIndex, dependencyMediators);
                    }
        ...
        private HashSet<int> MatchProduct(OldProduction product, int textToParseIndex, Dictionary<OldProduction, DependencyMediator> dependencyMediators) {
        ...
        /// <returns></returns>
        private SubMatchChain GetSubMatchChainFromCompletedProductsOrDependencyMediators(SubMatchChain.Entry entry, int indexInTextToParse, Dictionary<OldProduction, DependencyMediator> dependencyMediators, Stack<SubMatchChain.Entry> stack) {
        ...
        private int GetOrder(OldProduction a, OldProduction b) {
            return _precedence.Compare(a, b);
        }

        private SubMatchChain ChooseBestSubMatchChain(SubMatchChain a, SubMatchChain b, int indexInTextToParse, Dictionary<OldProduction, DependencyMediator> dependencyMediators, Stack<SubMatchChain.Entry> stack) {
            ...
            for (int subMatchIndex = 0; subMatchIndex < a.SubMatches.Count; subMatchIndex++) {
                OldProduction aSubProduct = a.SubMatches[subMatchIndex].Product;
                OldProduction bSubProduct = b.SubMatches[subMatchIndex].Product;
        ...
        private ParseResult Resultify(int indexInParsedText, OldProduction product, int length, Dictionary<OldProduction, Dictionary<int, ParseResult>>[] converted) {
            var indexConversions = converted[indexInParsedText];
            if (indexConversions == null) {
                indexConversions = new Dictionary<OldProduction, Dictionary<int, ParseResult>>();
        ...
        private ParseResult PrepareResult(OldProduction toMatch, int length) {
            var converted = new Dictionary<OldProduction, Dictionary<int, ParseResult>>[_textCodePoints.Length];
