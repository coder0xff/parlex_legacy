...
public class Nfa<TAlphabet, TAssignment> {
    ...
    /// </summary>
    /// <returns></returns>
    StateMap MakeStateMap(out Nfa<TAlphabet, Configuration> determinized) {
    ...
    static Nfa<TAlphabet, int> GenerateEquivalenceClassReducedDfa(Nfa<TAlphabet, Configuration> subsetConstructionDfa, Dictionary<Configuration, int> equivalenceClassLookup) {
    ...
    static ReducedStateMap ReduceStateMap(StateMap stateMap, Nfa<TAlphabet, Configuration> subsetConstructionDfa, out Nfa<TAlphabet, int> minimizedSubsetConstructionDfa) {
    ...
    IEnumerable<IEnumerable<State>> GetCycles() {
...
public class Nfa<TAlphabet>
{
    ...
    public Nfa(Nfa<TAlphabet> other)
    {
        ...
        foreach (State state in other.TransitionFunction.Keys)
        {
            foreach (TAlphabet symbol in other.TransitionFunction[state].Keys)
            {
                foreach (State toState in other.TransitionFunction[state][symbol])
                {
    ...
    public class Configuration : ReadOnlyHashSet<State>
    {
        public Configuration(IEnumerable<State> items)
            : base(items)
        {
        }
    }
    ...
    static IEnumerable<Cover> EnumerateCovers(Grid[] primeGrids, int firstGridIndex, Dictionary<Grid, HashSet<int>> gridToFlattenedIndicesSet, HashSet<int> flattenedIndicesWithTrue, int gridCount)
    {
        if (gridCount > primeGrids.Length - firstGridIndex)
    ...
    public bool IsEquivalent<TAssignment2>(Nfa<TAlphabet, TAssignment2> that)
    {
        ...
        processor.Run(pair =>
        {
            ...
            foreach (var inputSymbolAndStates in thisMinDfa.TransitionFunction[pair.Key])
            {
                ...
                var thisMinDfaNextState = inputSymbolAndStates.Value.First(); //deterministic, so only one state
                var thatMinDfaNextStates = thatMinDfa.TransitionFunction[pair.Value][thisMinDfaInputSymbol];
                if (thatMinDfaNextStates.Count != 1)
    ...
    struct Transition
    {
        ...
        public Transition(State fromState, TAlphabet inputSymbol, State toState)
            : this()
        {
            FromState = fromState;
            InputSymbol = inputSymbol;
    ...
    public void Insert(State at, Nfa<TAlphabet> require)
    {
        ...
        JaggedAutoDictionary<TAlphabet, List<State>> outgoingTransitions = new JaggedAutoDictionary<TAlphabet, List<State>>(_ => new List<State>());
        ...
        List<State> mappedAcceptStates = new List<State>();
